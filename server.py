...
app = Flask(__name__)
# Configure session
app.secret_key = 'flask-ecommerce-app'
app = Flask(__name__)
...
def login():
    if request.method == 'POST':
        ...
        cur.execute('SELECT users.id, users.role_id, users.username, users.password, role.role FROM users, role WHERE role.id=users.role_id AND users.username = %s', [username])
        user = cur.fetchone()
